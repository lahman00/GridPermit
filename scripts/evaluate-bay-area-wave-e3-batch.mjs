#!/usr/bin/env node
// Evaluates the third and final Bay Area sub-batch of the statewide
// expansion mission's Wave E: San Ramon, Dublin, Livermore, Union City,
// Newark, Antioch, Pittsburg, Brentwood, and Oakley. This completes
// Wave E (Bay Area). Reads each record and its existing validation
...
// output/bay-area-wave-e3-batch-evaluation.json.
//
// Usage: node scripts/evaluate-bay-area-wave-e3-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "bay-area-wave-e3-batch-evaluation.json");

const BAY_AREA_WAVE_E3_RECORD_IDS = [
	"ca-contra-costa-san-ramon-pge",
	"ca-alameda-dublin-pge",
	"ca-alameda-livermore-pge",
	"ca-alameda-union-city-pge",
	"ca-alameda-newark-pge",
	"ca-contra-costa-antioch-pge",
	"ca-contra-costa-pittsburg-pge",
	"ca-contra-costa-brentwood-pge",
	"ca-contra-costa-oakley-pge",
];
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"livermoreca.gov, pittsburgca.gov, and brentwoodca.gov all returned HTTP 403 to automated fetch this session, limiting those three records to weaker, search-engine-derived detail. sanramon.ca.gov, dublin.ca.gov, antiochca.gov, and oakleyca.gov were all fully reachable with strong detail.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"Caught a broken source URL for Brentwood's official_contacts entry (brentwoodca.gov/government/community-development/building-division/permits, confirmed HTTP 404 by the validator) before this batch was finalized — repointed to the already-verified-reachable SolarApp Submittals page rather than leaving a dead citation.",
			"Antioch and Brentwood were both confirmed to have no CCA — neither is listed among MCE's 16 current Contra Costa County member communities, unlike the majority of their neighbors researched across this wave.",
			"This batch completes Wave E (Bay Area) — 32 cities investigated across Alameda, Contra Costa, and San Francisco counties.",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of BAY_AREA_WAVE_E3_RECORD_IDS) {
