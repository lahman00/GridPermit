#!/usr/bin/env node
// Evaluates the fifth and final Northern California sub-batch of the
// statewide expansion mission's Wave K: Ukiah, Willits (Mendocino
// County) and Lakeport, Clearlake (Lake County). This is the final
// sub-batch of the entire statewide research queue. Reads each record
...
// output/northern-ca-wave-k5-batch-evaluation.json.
//
// Usage: node scripts/evaluate-northern-ca-wave-k5-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "northern-ca-wave-k5-batch-evaluation.json");

const NORTHERN_CA_WAVE_K5_RECORD_IDS = [
	"ca-mendocino-ukiah-eu",
	"ca-mendocino-willits-pge",
	"ca-lake-lakeport-pge",
	"ca-lake-clearlake-pge",
];
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"cityofukiah.com's Electric Utility page returned HTTP 503 and cityoflakeport.com returned HTTP 403 to automated fetch this session, limiting Ukiah's and Lakeport's records to search-engine-derived detail. A Willits-hosted PDF solar guide returned only corrupted/binary content, not extractable text. cityofwillits.org (general Building & Safety page) and clearlake.ca.us were both directly reachable.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"Ukiah was independently confirmed to operate its own municipal electric utility (a City department and NCPA member operating the Lake Mendocino Hydroelectric Plant) rather than assumed as PG&E territory, unlike neighboring Willits (also Mendocino County), which was independently confirmed as PG&E.",
			"No CCA was found serving any of the 4 cities in this batch — generation_supplier left null throughout.",
		],
	};
}
...
function buildBatchRecommendation(records) {
	...
	return {
		...
		note: "Applies the exact same deterministic thresholds already used for every prior batch (READY: completeness >=80% and score >=85 and 0 errors; LIMITED: completeness >=50% and score >=75 and 0 errors; else NOT_READY) — no record here was manually marked READY. This is the final sub-batch of the entire statewide research queue.",
	};
}

async function main() {
	const records = [];
	for (const id of NORTHERN_CA_WAVE_K5_RECORD_IDS) {
