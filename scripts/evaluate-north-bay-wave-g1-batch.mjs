#!/usr/bin/env node
// Evaluates the first North Bay sub-batch of the statewide expansion
// mission's Wave G: Petaluma, Rohnert Park, Cotati, Sebastopol,
// Healdsburg, Windsor, and Sonoma — Sonoma County. Reads each record
// and its existing validation report (does not re-run collection or
...
// output/north-bay-wave-g1-batch-evaluation.json.
//
// Usage: node scripts/evaluate-north-bay-wave-g1-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "north-bay-wave-g1-batch-evaluation.json");

const NORTH_BAY_WAVE_G1_RECORD_IDS = [
	"ca-sonoma-petaluma-pge",
	"ca-sonoma-rohnert-park-pge",
	"ca-sonoma-cotati-pge",
	"ca-sonoma-sebastopol-pge",
	"ca-sonoma-healdsburg-hed",
	"ca-sonoma-windsor-pge",
	"ca-sonoma-sonoma-pge",
];
...
function classifyUtilityType(utilityName) {
	...
	if (n.includes("WATER AND POWER") || n.includes("MUNICIPAL POWER") || n.includes("MUNICIPAL") || n.includes("LIGHT & WATER") || n.includes("PUBLIC UTILITIES") || n.includes("ELECTRIC DEPARTMENT") || n.includes("ELECTRIC UTILITY") || n.includes("SILICON VALLEY POWER") || n.includes("CPAU")) return "municipal";
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"cityofpetaluma.org returned HTTP 403 to automated fetch this session, limiting that record to weaker, search-engine-derived detail. rpcity.ca.gov, cotaticity.gov, cityofsebastopol.gov, healdsburg.gov, townofwindsor.ca.gov, and sonomacity.org were all fully reachable. sonomacleanpower.org's own dedicated service-area page returned HTTP 404 and its homepage did not list member cities directly, so Sonoma Clean Power's membership was recorded with reduced confidence (0.7) based on the CCA's own public self-description (public power provider for Sonoma and Mendocino counties, except Healdsburg and Ukiah) rather than a directly-fetched member list — flagged as a candidate for re-verification once a working member-list page is found.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"Introduces a fifth (and first Sonoma County) municipal-utility record this campaign: City of Healdsburg Electric Department, confirmed both by Healdsburg's own Electric Department page and by Sonoma Clean Power's own service description explicitly excluding Healdsburg from its territory — two independent confirmations of the same fact, a stronger basis than most other municipal-utility findings this campaign.",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of NORTH_BAY_WAVE_G1_RECORD_IDS) {
