#!/usr/bin/env node
// Evaluates the second Central Coast sub-batch of the statewide
// expansion mission's Wave J: Monterey, Pacific Grove,
// Carmel-by-the-Sea, Seaside, and Marina — all northern Monterey
// County. Reads each record and its existing validation report (does
// not re-run collection or validation itself) and produces one
// decision report, per-record and aggregate, using the exact same
// deterministic readiness framework already established by every
// prior batch evaluator. Read-only on data/localities/ and
// output/validation-reports/ — only ever writes
// output/central-coast-wave-j2-batch-evaluation.json.
//
// Usage: node scripts/evaluate-central-coast-wave-j2-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "central-coast-wave-j2-batch-evaluation.json");

const CENTRAL_COAST_WAVE_J2_RECORD_IDS = [
	"ca-monterey-monterey-pge",
	"ca-monterey-pacific-grove-pge",
	"ca-monterey-carmel-by-the-sea-pge",
	"ca-monterey-seaside-pge",
	"ca-monterey-marina-pge",
];
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"monterey.gov and cityofpacificgrove.gov both returned HTTP 403 to automated fetch this session, limiting Monterey's and Pacific Grove's records to search-engine-derived detail. ci.carmel.ca.us, ci.seaside.ca.us, and cityofmarina.org were all directly reachable, though the Marina and Seaside pages fetched were general building-permit pages without itemized solar-specific detail.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"Central Coast Community Energy (3CE) was reused from its already-established official-site citation (3cenergy.org/about-us/, verified in Wave J1) to confirm CCA coverage for all 5 cities in this Monterey County batch, consistent with the per-organization CCA verification pattern — 3CE's main office is in Monterey.",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of CENTRAL_COAST_WAVE_J2_RECORD_IDS) {
