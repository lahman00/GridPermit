#!/usr/bin/env node
// Evaluates the third LA County sub-batch of the statewide expansion
// mission's Wave A: Redondo Beach, Manhattan Beach, Hermosa Beach,
// Hawthorne, Gardena, Culver City, Santa Monica, Beverly Hills,
// West Hollywood, and Malibu. Reads each record and its existing
...
// output/la-county-wave-a3-batch-evaluation.json.
//
// Usage: node scripts/evaluate-la-county-wave-a3-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "la-county-wave-a3-batch-evaluation.json");

const LA_COUNTY_WAVE_A3_RECORD_IDS = [
	"ca-los-angeles-redondo-beach-sce",
	"ca-los-angeles-manhattan-beach-sce",
	"ca-los-angeles-hermosa-beach-sce",
	"ca-los-angeles-hawthorne-sce",
	"ca-los-angeles-gardena-sce",
	"ca-los-angeles-culver-city-sce",
	"ca-los-angeles-santa-monica-sce",
	"ca-los-angeles-beverly-hills-sce",
	"ca-los-angeles-west-hollywood-sce",
	"ca-los-angeles-malibu-sce",
];
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"manhattanbeach.gov, hermosabeach.gov, cityofhawthorne.org, culvercity.gov, and weho.org all returned HTTP 403 on every page attempted this batch, leaving those five records thin on city-specific permit process detail despite strong CCA sourcing for several of them.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"This batch benefited from a single, directly-fetched CalCCA press release ('Eight more Clean Power Alliance Communities Choose 100% Green Power') that explicitly named 21 CPA member communities including 7 cities in this exact batch (Redondo Beach, Manhattan Beach, Hawthorne, Culver City, Santa Monica, Beverly Hills, and Malibu, plus West Hollywood from the broader list) — reused as a single high-confidence source across multiple records rather than re-searched per city, the same efficiency pattern as the SCE incorporated-cities fact sheet in earlier batches. Manhattan Beach's record (written in an earlier sub-batch before this source was in hand) was retroactively upgraded from a weaker page-title-only citation to this directly-fetched confirmation.",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of LA_COUNTY_WAVE_A3_RECORD_IDS) {
