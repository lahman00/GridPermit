#!/usr/bin/env node
// Evaluates the second Riverside County sub-batch of the statewide
// expansion mission's Wave C: Banning, Menifee, Temecula, Murrieta,
// Lake Elsinore, Hemet, San Jacinto, and Beaumont. Reads each record and
...
// output/inland-empire-wave-c2-batch-evaluation.json.
//
// Usage: node scripts/evaluate-inland-empire-wave-c2-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "inland-empire-wave-c2-batch-evaluation.json");

const INLAND_EMPIRE_WAVE_C2_RECORD_IDS = [
	"ca-riverside-banning-beu",
	"ca-riverside-menifee-sce",
	"ca-riverside-temecula-sce",
	"ca-riverside-murrieta-sce",
	"ca-riverside-lake-elsinore-sce",
	"ca-riverside-hemet-sce",
	"ca-riverside-san-jacinto-sce",
	"ca-riverside-beaumont-sce",
];
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"temeculaca.gov returned a connection error (ECONNRESET) to automated fetch this session, limiting that record to weaker, search-engine-derived detail. banningca.gov's dedicated solar page returned a connection error on the first attempt but succeeded on a retry via a different page path (banningca.gov/573/Solar). menifee.ca.gov, murrietaca.gov, lake-elsinore.org, hemetca.gov, sanjacintoca.gov, and beaumontpd.org were all fully reachable.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"Introduces a third municipal-utility record this campaign (Banning Electric Utility, a Southern California Public Power Authority member), confirmed via Banning's absence from SCE's own service-area fact sheet, consistent with the pattern already established for Burbank, Glendale, Anaheim, and Riverside.",
			"Hemet is a Western Community Energy (WCE) member city, but WCE deregistered with the CPUC and dissolved via Chapter 9 bankruptcy in 2021 — generation_supplier is left null rather than recording a defunct CCA relationship as current, the same handling already applied to the WCE member cities in Wave C1 (Eastvale, Jurupa Valley, Norco, Perris).",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of INLAND_EMPIRE_WAVE_C2_RECORD_IDS) {
