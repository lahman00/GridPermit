#!/usr/bin/env node
// Evaluates the fourth Central Coast sub-batch of the statewide
// expansion mission's Wave J: San Luis Obispo, Paso Robles,
// Atascadero, Morro Bay, Pismo Beach, Arroyo Grande, and Grover
// Beach — all San Luis Obispo County. Reads each record and its
...
// output/central-coast-wave-j4-batch-evaluation.json.
//
// Usage: node scripts/evaluate-central-coast-wave-j4-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "central-coast-wave-j4-batch-evaluation.json");

const CENTRAL_COAST_WAVE_J4_RECORD_IDS = [
	"ca-san-luis-obispo-san-luis-obispo-pge",
	"ca-san-luis-obispo-paso-robles-pge",
	"ca-san-luis-obispo-atascadero-pge",
	"ca-san-luis-obispo-morro-bay-pge",
	"ca-san-luis-obispo-pismo-beach-pge",
	"ca-san-luis-obispo-arroyo-grande-pge",
	"ca-san-luis-obispo-grover-beach-pge",
];
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"slocity.org and prcity.com both returned HTTP 403 to automated fetch this session, limiting San Luis Obispo's and Paso Robles's records to search-engine-derived detail. atascadero.org, morrobayca.gov, pismobeach.org, arroyogrande.org, and groverbeach.org were all directly reachable, with several yielding comprehensive, directly-fetched solar-specific detail (fees, timelines, eligibility criteria).",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"Central Coast Community Energy (3CE) was reused from its already-established official-site citation (3cenergy.org/about-us/, verified in Wave J1) to confirm CCA coverage for all 7 cities in this San Luis Obispo County batch, consistent with the per-organization CCA verification pattern.",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of CENTRAL_COAST_WAVE_J4_RECORD_IDS) {
