#!/usr/bin/env node
// Evaluates the third North Bay sub-batch of the statewide expansion
// mission's Wave G: Vallejo, Benicia, Fairfield, Suisun City, Vacaville,
// and Dixon — Solano County. Reads each record and its existing
...
// output/north-bay-wave-g3-batch-evaluation.json.
//
// Usage: node scripts/evaluate-north-bay-wave-g3-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "north-bay-wave-g3-batch-evaluation.json");

const NORTH_BAY_WAVE_G3_RECORD_IDS = [
	"ca-solano-vallejo-pge",
	"ca-solano-benicia-pge",
	"ca-solano-fairfield-pge",
	"ca-solano-suisun-city-pge",
	"ca-solano-vacaville-pge",
	"ca-solano-dixon-pge",
];
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"fairfield.ca.gov and cityofvacaville.gov both returned HTTP 403 to automated fetch this session, limiting those two records to weaker, search-engine-derived detail. vallejo.gov, ci.benicia.ca.us, suisun.com, and cityofdixonca.gov were all fully reachable.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"This batch completes Solano County coverage in this campaign. MCE serves only 3 of the 6 cities researched (Benicia, Fairfield, Vallejo) — Suisun City, Vacaville, and Dixon are all confirmed absent from MCE's current member list, so generation_supplier was left null for all three rather than assumed.",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of NORTH_BAY_WAVE_G3_RECORD_IDS) {
