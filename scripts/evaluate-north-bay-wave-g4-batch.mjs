#!/usr/bin/env node
// Evaluates the fourth and final North Bay sub-batch of the statewide
// expansion mission's Wave G: San Rafael, Novato, Mill Valley,
// Sausalito, Larkspur, Corte Madera, Tiburon, Fairfax, San Anselmo, and
// Ross — Marin County. This completes Wave G (North Bay). Reads each
// record and its existing validation report (does not re-run collection
// or validation itself) and produces one decision report, per-record
// and aggregate, using the exact same deterministic readiness framework
...
// output/north-bay-wave-g4-batch-evaluation.json.
//
// Usage: node scripts/evaluate-north-bay-wave-g4-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "north-bay-wave-g4-batch-evaluation.json");

const NORTH_BAY_WAVE_G4_RECORD_IDS = [
	"ca-marin-san-rafael-pge",
	"ca-marin-novato-pge",
	"ca-marin-mill-valley-pge",
	"ca-marin-sausalito-pge",
	"ca-marin-larkspur-pge",
	"ca-marin-corte-madera-pge",
	"ca-marin-tiburon-pge",
	"ca-marin-fairfax-pge",
	"ca-marin-san-anselmo-pge",
	"ca-marin-ross-pge",
];
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"novato.gov returned HTTP 403 to automated fetch this session (after confirming a 301 redirect from novato.org), limiting that record to weaker, search-engine-derived detail. All nine other Marin County city/town sites (cityofsanrafael.org, cityofmillvalley.gov, sausalito.gov, cityoflarkspur.org, cortemadera.gov, townoftiburon.gov after a redirect, townoffairfaxca.gov, sananselmo.gov, townofrossca.gov) were reachable, though Fairfax's and Ross's own solar-specific content was thinner than the others.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"This batch completes Marin County coverage and Wave G (North Bay) overall. All 10 cities researched are confirmed current MCE members via MCE's own service-area page (which was directly re-verified once this session and reused as the citation across all 22 North Bay cities researched in Wave G, spanning Marin, Napa, and Solano counties).",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of NORTH_BAY_WAVE_G4_RECORD_IDS) {
