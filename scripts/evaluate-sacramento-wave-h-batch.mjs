#!/usr/bin/env node
// Evaluates the final Sacramento-region sub-batch of the statewide
// expansion mission's Wave H: Galt, Lincoln, Auburn, and Winters. This
// completes Wave H (Sacramento region). Reads each record and its
...
// output/sacramento-wave-h-batch-evaluation.json.
//
// Usage: node scripts/evaluate-sacramento-wave-h-batch.mjs
...
const OUT_PATH = path.join(REPO_ROOT, "output", "sacramento-wave-h-batch-evaluation.json");

const SACRAMENTO_WAVE_H_RECORD_IDS = [
	"ca-sacramento-galt-smud",
	"ca-placer-lincoln-pge",
	"ca-placer-auburn-pge",
	"ca-yolo-winters-pge",
];
...
function classifyUtilityType(utilityName) {
	...
	if (n.includes("WATER AND POWER") || n.includes("MUNICIPAL POWER") || n.includes("MUNICIPAL") || n.includes("LIGHT & WATER") || n.includes("PUBLIC UTILITIES") || n.includes("ELECTRIC DEPARTMENT") || n.includes("ELECTRIC UTILITY") || n.includes("SILICON VALLEY POWER") || n.includes("CPAU") || n.includes("SMUD") || n.includes("SACRAMENTO MUNICIPAL")) return "municipal";
...
function buildAggregate(records) {
	...
	return {
		...
		recurring_source_access_problems: [
			"cityofgalt.org returned HTTP 403 to automated fetch on every attempt this session, limiting that record's permitting-specific detail; lincolnca.gov, auburn.ca.gov, and cityofwinters.org were all fully reachable, though Winters's own page had thin solar-specific content.",
		],
		...
		trust_rule_corrections_made_during_collection: [
			"This batch completes Wave H (Sacramento region). Confirms Galt is SMUD (not PG&E) territory via SMUD's own Jurisdiction Contacts page, which directly lists the City of Galt — a stronger citation than the initial search-engine synthesis, which was corrected before finalizing the record. Confirms Winters is served by Valley Clean Energy (VCE), the Yolo County CCA already established for neighboring Davis and Woodland in this dataset, which welcomed Winters in January 2021.",
		],
	};
}
...
async function main() {
	const records = [];
	for (const id of SACRAMENTO_WAVE_H_RECORD_IDS) {
